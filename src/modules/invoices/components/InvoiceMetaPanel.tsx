...
export default function InvoiceMetaPanel({ name, onCollapse }: InvoiceMetaPanelProps) {
  ...
  const handleAssign = () => {
    ...
    if (!value) return
    run(() => invoiceService.assignUserToDoc(name, value)).then(() => setAssignee(""))
  }

  const handleRemove = (userId: string) => run(() => invoiceService.unassignUserFromDoc(name, userId))

  const handleDone = (userId: string) => run(() => invoiceService.completeOwnAssignment(name, userId))
  ...
  const handleAddTag = (raw: string) => {
    ...
    run(() => invoiceService.addTagToDoc(name, value)).then(() => {
      setTagQuery("")
      setTagOpen(false)
    })
  }
