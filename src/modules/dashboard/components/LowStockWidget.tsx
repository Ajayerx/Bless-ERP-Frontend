  ...
  return (
    ...
    >
      {data.map((item) => (
        ...
        >
          <div className="w-9 h-9 rounded-[8px] bg-danger-100 flex items-center justify-center shrink-0">
            <Package size={16} className="text-danger-600" />
          </div>
          ...
          <span className="inline-flex px-2 py-0.5 text-[10px] font-semibold rounded-full bg-danger-100 text-danger-700 shrink-0">
            {item.stock} left
          </span>
        </div>
      ))}
