...
// ERPNext's form sidebar (Assignments + Tags) for the Customer doctype. Mirrors
...
export default function CustomerMetaPanel({ name, initialDocinfo, onCollapse }: CustomerMetaPanelProps) {
  ...
  const handleAssign = () => {
    ...
    if (!value) return
    run(() => customerService.assignUserToDoc(name, value)).then(() => setAssignee(""))
  }

  const handleRemove = (userId: string) => run(() => customerService.unassignUserFromDoc(name, userId))

  const handleDone = (userId: string) => run(() => customerService.completeOwnAssignment(name, userId))
  ...
  const handleAddTag = (raw: string) => {
    ...
    run(() => customerService.addTagToDoc(name, value)).then(() => {
      setTagQuery("")
      setTagOpen(false)
    })
  }

  const handleRemoveTag = (tag: string) => run(() => customerService.removeTagFromDoc(name, tag))

  // Existing-Tag suggestions (TagEditor autocompletes from the Tag master).
  useEffect(() => {
    let cancelled = false
    const timer = setTimeout(() => {
      customerService.searchTags(tagQuery).then((suggestions) => {
