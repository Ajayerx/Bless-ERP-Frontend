...
export default function CustomerImportModal({ open, onClose, onComplete }: Props) {
  ...
  const handleImport = async () => {
    ...
    setError("")
    try {
      const res = await customerService.importFromCsv(file)
      setResult(res)
    } catch (e) {
  ...
  return (
    <Modal open={open} onClose={handleClose} title="Import Customers" size="md">
        ...
        {result ? (
          <div className="space-y-3">
            <div className="flex items-center gap-3 p-3 bg-success-50 border border-success-100 rounded-[10px]">
              ...
              <div>
                <p className="text-sm font-semibold text-heading">{result.success} customer{result.success !== 1 ? "s" : ""} imported</p>
                {result.failed > 0 && (
          ...
          <div>
            <p className="text-sm text-muted mb-3">
              Upload a CSV file with columns: <strong>customer_name</strong> (required), customer_type, customer_group, territory
            </p>
            <input
              ref={fileRef}
              type="file"
