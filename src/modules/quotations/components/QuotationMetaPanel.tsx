...
interface QuotationMetaPanelProps {
  name: string
...
// the Quotation module's service. Mirrors sidebar/assign_to.js and
// ui/tag_editor.js; every mutation refetches get_docinfo from the server.
export default function QuotationMetaPanel({ name, onCollapse }: QuotationMetaPanelProps) {
  const { user } = useAuth()
  ...
  const reload = useCallback(async () => {
    setLoading(true)
    try {
      const doc = await quotationService.getDocInfo(name)
  ...
  const handleAssign = () => {
    ...
    if (!value) return
    run(() => quotationService.assignUserToDoc(name, value)).then(() => setAssignee(""))
  }

  const handleRemove = (userId: string) => run(() => quotationService.unassignUserFromDoc(name, userId))

  const handleDone = (userId: string) => run(() => quotationService.completeOwnAssignment(name, userId))
  ...
  const handleAddTag = (raw: string) => {
    ...
    run(() => quotationService.addTagToDoc(name, value)).then(() => {
      setTagQuery("")
      setTagOpen(false)
    })
  }

  const handleRemoveTag = (tag: string) => run(() => quotationService.removeTagFromDoc(name, tag))

  // Existing-Tag suggestions (TagEditor autocompletes from the Tag master).
  useEffect(() => {
    let cancelled = false
    const timer = setTimeout(() => {
      quotationService.searchTags(tagQuery).then((suggestions) => {
