...
export default function SendPaymentEmailDialog({
  open,
  onOpenChange,
  paymentName,
  contactEmail,
  ...
  const [subject, setSubject] = useState(`Payment ${paymentName}`)
  const [content, setContent] = useState(
    `Dear Sir/Madam,\n\nPlease find attached payment entry ${paymentName}.\n\nBest regards,\nBlessERP`
  )
  ...
  useEffect(() => {
    if (open) {
      ...
      setSubject(`Payment ${paymentName}`)
      setContent(
        `Dear Sir/Madam,\n\nPlease find attached payment entry ${paymentName}.\n\nBest regards,\nBlessERP`
      )
      ...
      setSent(false)
      setError("")
      paymentService.getPrintFormats().then(setPrintFormats).catch(() => {})
    }
  }, [open, paymentName, contactEmail])
  ...
  const handleSend = async () => {
    ...
    setError("")
    try {
      await paymentService.sendEmail(paymentName, {
