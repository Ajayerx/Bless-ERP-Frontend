...
import { paymentService, type DocInfoAssignment, type DocInfoUserInfo } from "../services"
...
// ERPNext's form sidebar (Assignments + Tags) as a compact left column. Mirrors
// sidebar/assign_to.js (avatars, add/remove/complete) and ui/tag_editor.js
// (chips + add/remove with existing-Tag suggestions). Data comes from
// frappe.desk.form.load.get_docinfo; every mutation refetches from the server.
export default function PaymentMetaPanel({ name, onCollapse }: PaymentMetaPanelProps) {
  const { user } = useAuth()
  ...
  const reload = useCallback(async () => {
    setLoading(true)
    try {
      const doc = await paymentService.getDocInfo(name)
  ...
  const handleAssign = () => {
    ...
    if (!value) return
    run(() => paymentService.assignUserToDoc(name, value)).then(() => setAssignee(""))
  }

  const handleRemove = (userId: string) => run(() => paymentService.unassignUserFromDoc(name, userId))

  const handleDone = (userId: string) => run(() => paymentService.completeOwnAssignment(name, userId))
  ...
  const handleAddTag = (raw: string) => {
    ...
    run(() => paymentService.addTagToDoc(name, value)).then(() => {
      setTagQuery("")
      setTagOpen(false)
    })
  }

  const handleRemoveTag = (tag: string) => run(() => paymentService.removeTagFromDoc(name, tag))

  // Existing-Tag suggestions (TagEditor autocompletes from the Tag master).
  useEffect(() => {
    let cancelled = false
    const timer = setTimeout(() => {
      paymentService.searchTags(tagQuery).then((suggestions) => {
