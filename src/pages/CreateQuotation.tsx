...
import { quotationService, customerService, type Customer } from "@/services"
...
export default function CreateQuotation() {
  ...
  const selectCustomer = (c: Customer) => {
    setCustomerId(c.id); setCustomerName(c.name); setCustomerContact(c.contactName); setBillTo(c.billingAddress)
  ...
  const handleSave = async () => {
    ...
    try {
      await quotationService.create({
        customerId, customerName, customerContact, billTo, issueDate, validUntil, notes,
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted mt-0.5">Create a new sales quotation for a customer.</p>
            </div>
          </div>
